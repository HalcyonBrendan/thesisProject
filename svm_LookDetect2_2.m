...
handle = sprintf('../analysis/workspaces/%s/LOO_detANOVA_NN_240_170316_6264.mat',folder);
...
boxCons = [.003];numBoxCons = length(boxCons);
pVals = [.0001 .00001]; numPs = length(pVals);
classProbs = [.95 .05; .9 .1]; numCPs = size(classProbs,1);
...
res=s(results(:,2,1,1,1,1,:));
sco=s(scores(:,2,1,1,1,1,:));
