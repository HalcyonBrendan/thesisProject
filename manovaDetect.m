...
for chan = 1:numChannels
    ...
    for trial = 1:numSacs
        ...
        for l = 1:numPeriods
            ...
            [~,sacDetectPVals(chan,trial,l)] = manova1(bandVals,moveVals);
        end
    end
    % Purs next
    for trial = 1:numPurs
        ...
        for l = 1:numPeriods
            ...
            moveVals = repmat([ones(length(moveIdxs),1);zeros(length(noMoveIdxs(l,:)),1)],numTrainTrials,1);
            [~,purDetectPVals(chan,trial,l)] = manova1(bandVals,moveVals);
        end
    end
...
handle = sprintf('../analysis/workspaces/%s/LOO_detANOVA_NN_100216.mat',folder);
save(handle,'sacDetectPVals','purDetectPVals','sacSigChans','purSigChans','sacSigPVals','purSigPVals','times','tIdxs','fBounds','pVal','validSacs','validPurs');
