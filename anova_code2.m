...
fprintf('Beginning anova_code2.m!\n');
...
sacSigPVals = s(sum(sacTuningPVals(1:numChannels,:,:)<.1,1));
sacSigChans = cell(numFreqBands,numTimeSteps);
purSigPVals = s(sum(purTuningPVals(1:numChannels,:,:)<.1,1));
purSigChans = cell(numFreqBands,numTimeSteps);
for i = 1:numFreqBands
    for j = 1:numTimeSteps
        sacSigChans{i,j}=find(sacTuningPVals(1:numChannels,i,j)<.1);
        purSigChans{i,j}=find(purTuningPVals(1:numChannels,i,j)<.1);
    end
end

handle = sprintf('../analysis/workspaces/%s/tunedChans_90_NN_dec1415.mat',folder);
