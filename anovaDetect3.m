...
if strcmp(type,'sacs')
    validSacs = validTrials(info(validTrials,4)==65 & ~isnan(TT.T65sac(validTrials)));
    numSacs = length(validSacs); numPurs = 0;
elseif strcmp(type,'purs')
    validPurs = validTrials(info(validTrials,4)==66 & ~isnan(TT.T66on(validTrials)));
...
moveIdxs = 77:81;
% Create vector that classifies each time step as "MOVE"=1 or "NO MOVE"=0
move = zeros(length(t),1);
move(moveIdxs) = 1;
% Choose which times to use for ANOVA
%tIdxs = [41:2:63 64:75 moveIdxs];
tIdxs = [69:73 moveIdxs];
...
handle = sprintf('../analysis/workspaces/%s/detANOVA_170416_6973v7781_2.mat',folder);
