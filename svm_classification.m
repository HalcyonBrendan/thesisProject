...
%
% Classifies eye movement direction using support vector machine

fprintf('Beginning svm_classification.m!\n');
c=clock; fprintf('%.0f:%.0f\n',c(4),c(5));
clear all;

%%%%%%%%%% TO SET %%%%%%%%%%%
% Choose which channels to look at
mipChans = 1:18; pmdChans = 37:48; allChans = 1:48;
channels = mipChans;
chanType = 'mip';%'pmd','both'
numChannels = length(channels);
pVals = [.05 .01 .005 .0005];
numPs = length(pVals);
...
yOutProb = zeros(numTrials,numPs,length(unique(labels)));
...
for l = 1:numTrials
    ...
    for p = 1:numPs
        ...
        % Classify
        fprintf('Training SVM Classifier and Classifying!\n');
        
        % Train svm model with X samples matrix
        SVMModel =  svm_train(X(:,1:trainInd-1),y);
        % Classify samples given in Xtest according to trained model and
        % keep track according to pVal(p) and left out trial l
        [yOut(l,p),yOutProb(l,p,:)] =  svm_test(SVMModel,Xtest(1:testInd-1));
...
percentChance = 1/length(SVMModel.ClassNames);

%%%%%%%%%%%% TO SET %%%%%%%%%%%
descript = 'Time: 701am, first svm class run, 300ms spects';
handle = sprintf('../analysis/workspaces/%s/mip_svmDirSacClass_LOO_160216.mat',folder);
