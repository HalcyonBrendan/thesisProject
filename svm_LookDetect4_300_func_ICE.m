function movesCorrPerc = svm_LookDetect4_300_func( folder,spects,valChannels,t,f,numChans,chType,lookType,pVal,polyOrd,boxCon,classProb,testIdxs,testTrials,randNum,iteration )
% svm_LookDetect2.m
% Brendan Thorn - 02/16
...
falseNegs = numTestTrials - truePos(p,pCs,bCs);
...
% tit = sprintf('%s %s Probs SVM 240416',chType,lookType);
...
% tit = sprintf('%s %s Detects SVM 240416',chType,lookType);
% title(tit);line([-.15 -.15],[0 numTestTrials],'Color','r','LineWidth',2);line([0 0],[0 numTestTrials],'Color','g','LineWidth',2);line([-.05 -.05],[0 numTestTrials],'Color','c','LineWidth',2);

handle = sprintf('../analysis/workspaces/%s/%s%s_optRun_%dchs_240416_%d_%d.mat',folder,type,chanType,numChans,randNum,iteration);
save(handle,'adjResults','bounds','boxCons','c','channels','chanType','f','falseNegs','falsePos','fBounds','folder','fpRate','handle','I','MCC','movesCorrPerc','noMovesCorrPerc','pVals','results','S','scores','t','testIdxs','testMoves','testMovesIdxs','testTimes','testTrials','testTrialY','threshValue','trainMoves','trainMovesIdxs','trainTimes','trueNegs','truePos','tunedChanCount','type','valChannels','valTrials');
