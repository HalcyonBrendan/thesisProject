function [ ] = svm_LookDetect2_300_func( folder,numChans,chType,lookType,pVal,polyOrd,boxCon,classProb,randNum )
% svm_LookDetect2.m
% Brendan Thorn - 02/16
...
classProbs = classProb; numCPs = size(classProbs,1);
...
testIdxs = max(round(maxTestIdx/2),40)+1:maxTestIdx; %% <-- SET THIS
...
title('MIP Pursuit Detects -- SVM');line([-.15 -.15],[0 numTestTrials],'Color','r','LineWidth',2);line([0 0],[0 numTestTrials],'Color','g','LineWidth',2);line([-.20 -.2],[0 numTestTrials],'Color','c','LineWidth',2);line([-.1 -.1],[0 numTestTrials],'Color','c','LineWidth',2);
...
handle = sprintf('../analysis/workspaces/%s/%s%s_optRun_%dchs_210316_%d.mat',folder,type,chanType,numChans,randNum);
save(handle);
