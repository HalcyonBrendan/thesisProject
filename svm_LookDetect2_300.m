...
polyOrds = [7]; numPolys = length(polyOrds);
boxCons = [.01];numBoxCons = length(boxCons);
pVals = [.0000001]; numPs = length(pVals);
classProbs = [.9 .1]; numCPs = size(classProbs,1);
...
testIdxs = round(maxTestIdx/2)+1:maxTestIdx; %% <-- SET THIS
...
title('Saccade Probability -- SVM 210316');line([-.15 -.15],[0 numTestTrials],'Color','r','LineWidth',2);line([0 0],[0 numTestTrials],'Color','g','LineWidth',2);line([-.2 -.2],[0 numTestTrials],'Color','c','LineWidth',2);line([-.1 -.1],[0 numTestTrials],'Color','c','LineWidth',2);
...
title('MIP Pursuit Detects -- SVM (MCC=0.277)');line([-.15 -.15],[0 numTestTrials],'Color','r','LineWidth',2);line([0 0],[0 numTestTrials],'Color','g','LineWidth',2);line([-.20 -.2],[0 numTestTrials],'Color','c','LineWidth',2);line([-.1 -.1],[0 numTestTrials],'Color','c','LineWidth',2);

% clear allChans ax bCs chan colInd cPs detAnova Events fb freqBands i info LFP mipChans oFs p pCs pmdChans spect spects SVMModel testCount testY testX trainIdxs trainTrials trainY trainX TT validTrials maxMCCval maxMCCidx;
% descript = 'opt param run: 300ms windows, 12 mip chans';
% handle = sprintf('../analysis/workspaces/%s/%s%s_optRun_210316.mat',folder,type,chanType);
% save(handle);
