...
fprintf('Beginning classification2_2.m!\n');
clear all;
...
% TO SET
numRuns = 50;
pVals = [.99:-.05:.29 .29:-.04:.07 .06:-.01:.02];% .009:-.001:.001 .0005];
...
% Break off some trials for performing ANOVA to find tuned channels
anovaLength = round(2/4*length(validTrials));
anovaTrials = validTrials(1:anovaLength);
validTrials = validTrials(anovaLength+1:end);
...
anova_code3_func(anovaTrials, type, .05, 95, folder);
% Load tuning information produced by anova_code2.m
handle = sprintf('../analysis/workspaces/%s/%sTunedChans_%d_NN_jan1816.mat',folder,type,95);
...
numTrainTrials = round(3/4*numTrials);
...
% %%%%%%%%%%%% TO SET %%%%%%%%%%%
% handle = sprintf('../analysis/workspaces/%s/dirSacClass_moreAnova_50r_180116.mat',folder);
% save(handle,'answerCheck','numCorr','percentCorr','percentChance','numTrainTrials','numTestTrials', ...
%     'numRuns','numPs','pVals','channels','type','anovaTrials','validTrials');
...
title('Sac Direction Prediction Rate vs Confidence Level (50 Runs)');
xlim([.02 .99]);
ax = gca; set(ax,'XScale','log');
pValues = [.02, .05, .1, .2, .3, .5, .99];
ax.XTick = pValues;
